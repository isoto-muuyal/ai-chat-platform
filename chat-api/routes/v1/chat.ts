...
router.post('/stream', async (req: Request, res: Response) => {
  ...
  const sanitizeResult = promptSanitizer.check(message);
  if (sanitizeResult.blocked) {
    logger.warn({ accountNumber, reason: sanitizeResult.reason }, 'prompt blocked by sanitizer');
    const funnyReply = promptSanitizer.blockedMessage();
    res.setHeader('Content-Type', 'text/event-stream');
    res.setHeader('Cache-Control', 'no-cache');
    res.setHeader('Connection', 'keep-alive');
    res.write(`event: meta\n`);
    res.write(`data: ${JSON.stringify({ ok: false, blocked: true, receivedAt: new Date().toISOString(), conversationId })}\n\n`);
    res.write(`event: token\n`);
    res.write(`data: ${JSON.stringify({ text: funnyReply })}\n\n`);
